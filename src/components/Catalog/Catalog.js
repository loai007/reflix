...
class Catalog extends Component {
  ...
  rent = (movieID) => {
    ...
    if (movies[movieID].isRented) {
      ...
      this.setState({ budget: this.state.budget + 20 });
    } else if (this.state.budget > 20) {
  ...
  generateMovies(movie) {
    return <Movie movie={movie} rentFunc={this.rent} />;
  }
  generateRentedMovie(movie) {
    if (movie.isRented) return <Movie movie={movie} rentFunc={this.rent} />;
  }
  render() {
    let isAnyMovieRented = this.checkRentedMovies();
    ...
    return (
      <div>
          ...
          {isAnyMovieRented ? (
            <div>
              ...
              {this.state.movies.map((movie) =>
                this.generateRentedMovie(movie)
              )}
              <hr />
            </div>
          ) : (
        ...
        <div>
          <div>
            <div>Catalog: </div>
            {this.state.movies.map((movie) => this.generateMovies(movie))}
          </div>
        </div>
      </div>
    );
